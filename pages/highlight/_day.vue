...
export default class Highlights extends mixins(ApiMixin, DateMixin) {
  ...
  validate (ctx: Context) {
    if (['legal-notice', 'homepage', 'contact'].find(route => route === ctx.route.name)) {
      return true
    }
    ...
    const greaterThanMinDate = setTimezone(new Date(Date.parse('1 Dec 2020 00:00:00 GMT'))) <= selectedDate
